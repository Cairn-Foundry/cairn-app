...
type PathsToLeaf<T, Prefix extends string = ''> =
  T extends string | ((...args: never[]) => string)
    ? Prefix
    : {
...
const currentDict = dictionaries[loadLocale()] as unknown as Record<string, unknown>;

// --- Locale metadata ---
export const LOCALE_META: Record<Locale, { nativeName: string; englishName: string }> = {
  en: { nativeName: 'English',  englishName: 'English' },
  fr: { nativeName: 'Français', englishName: 'French'  },
};

// --- Public API ---
export function t(key: TranslationKey): string | ((...args: never[]) => string) {
  return getNestedValue(currentDict, key) as string | ((...args: never[]) => string);
}
