<script lang="ts">
  ...
  const SAVE_ON_OPTIONS: { value: string; label: string; desc: string }[] = [
...
<svelte:window on:keydown={handleRecordKeydown} on:keydown={(e) => { if (e.key === 'Escape' && saveOnOpen) saveOnOpen = false; }} />
                ...
                <div class="so-menu" role="listbox">
                  {#each SAVE_ON_OPTIONS as opt}
                    ...
                    <button
                      ...
                      on:click={() => { settings.save({ saveOn: opt.value as any }); saveOnOpen = false; }}
                    >
