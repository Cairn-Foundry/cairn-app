...
>
  <div class="modal del-modal" on:click|stopPropagation role="presentation">
    <div class="modal-head">
      <div>
        <div class="step-count">{t('mcp.delete.heading')}</div>
        <h3>{(t('mcp.delete.title') as (n: string) => string)(name)}</h3>
      </div>
    ...
    <div class="modal-body">
      <p class="del-desc">{t('mcp.delete.description')}</p>
      <code class="del-path selectable">{sourcePath}</code>
    </div>
    <div class="modal-foot">
      ...
      <button class="btn danger" on:click={() => dispatch('confirm')}>
        <Icon name="trash" size={14}/> {t('mcp.delete.confirm')}
      </button>
    </div>
  </div>
</div>
