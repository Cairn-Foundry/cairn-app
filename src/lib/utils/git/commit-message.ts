...
/**
 * `{{ticket}}` is the one placeholder a template may carry. It expands to an
 * instruction when the instance has a ticket and to nothing when it does not,
 * so a template can mention it unconditionally.
 */
export function renderCommitPrompt(template: string, ticketId: string): string {
	const clause = ticketId ? ` End the subject with \`, ${ticketId}\`.` : "";
	return template.replaceAll("{{ticket}}", clause);
}
