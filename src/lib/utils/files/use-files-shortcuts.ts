...
import { flattenVisible, flattenToNodes, parentPathOf, basename } from './files-tree';
...
export interface FilesShortcutsContext {
  ...
  getFileClipboard: () => { nodes: FileNode[]; srcWorktreePath: string; op: 'copy' | 'cut' } | null;
  setFileClipboard: (cb: { nodes: FileNode[]; srcWorktreePath: string; op: 'copy' | 'cut' } | null) => void;
  ...
  pasteClipboard: (
    clipboard: { nodes: FileNode[]; srcWorktreePath: string; op: 'copy' | 'cut' },
    targetNode: FileNode | null,
    wtp: string,
  ) => Promise<void>;
  deleteAtPaths: (paths: string[]) => Promise<void>;
  startEdit: (state: { type: 'rename' | 'new-file' | 'new-dir'; node: FileNode | null; parentPath: string; value: string }) => void;
...
export function makeFilesKeyHandler(ctx: FilesShortcutsContext) {
  return async function handleGlobalKey(e: KeyboardEvent) {
    ...
    if (matchesShortcut(e, sc.quickOpen))    { e.preventDefault(); ctx.openQuickOpen(); }
    if (matchesShortcut(e, sc.searchFiles))  { e.preventDefault(); ctx.toggleSearchPanel(); }
    if (matchesShortcut(e, sc.fontSizeUp))   { e.preventDefault(); ctx.bumpFontSize(+1); }
    if (matchesShortcut(e, sc.fontSizeDown)) { e.preventDefault(); ctx.bumpFontSize(-1); }
    if (matchesShortcut(e, sc.fontSizeReset)){ e.preventDefault(); ctx.resetFontSize(); }
    if (matchesShortcut(e, sc.splitEditor))  { e.preventDefault(); ctx.toggleSplit(); }
    ...
    if (matchesShortcut(e, sc.reopenClosedTab)) { e.preventDefault(); ctx.reopenClosedTab(); }
    ...
    if (matchesShortcut(e, sc.prevTab)) {
      e.preventDefault();
      const len = ctx.getTabsLengthFor(0);
      if (len > 1) ctx.switchTab(0, (ctx.getActiveTabIdxFor(0) - 1 + len) % len);
    }
    if (matchesShortcut(e, sc.tabHistoryBack))    { e.preventDefault(); ctx.tabHistoryBack(); }
    if (matchesShortcut(e, sc.tabHistoryForward)) { e.preventDefault(); ctx.tabHistoryForward(); }
    ...
    if ((IS_MAC ? e.metaKey : e.ctrlKey) && !e.shiftKey && !e.altKey && /^[1-9]$/.test(e.key)) {
      ...
      if (idx < ctx.getTabsLengthFor(0)) { e.preventDefault(); ctx.switchTab(0, idx); }
    }

    // Editing
    if (matchesShortcut(e, sc.saveFile)) { e.preventDefault(); ctx.saveActivePane(activePane); }

    // View
    if (matchesShortcut(e, sc.toggleSidebar))   { e.preventDefault(); ctx.toggleSidebar(); }
    if (matchesShortcut(e, sc.commandPalette))  { e.preventDefault(); ctx.openCommandPalette(); }
    if (matchesShortcut(e, sc.openSettings))    { e.preventDefault(); ctx.openSettings(); }
    ...
    if (matchesShortcut(e, sc.treeSelectAll)) {
      e.preventDefault();
      ctx.setMultiSelected(new Set(flattenVisible(tree, expanded).map(n => n.path)));
    }
    if (matchesShortcut(e, sc.treeCopy) && multiSelected.size > 0 && wtp) {
      e.preventDefault();
      ctx.setFileClipboard({ nodes: flattenToNodes(tree, multiSelected), srcWorktreePath: wtp, op: 'copy' });
    }
    if (matchesShortcut(e, sc.treeCut) && multiSelected.size > 0 && wtp) {
      e.preventDefault();
      ctx.setFileClipboard({ nodes: flattenToNodes(tree, multiSelected), srcWorktreePath: wtp, op: 'cut' });
    }
    ...
    if (matchesShortcut(e, sc.treeRename) && multiSelected.size === 1) {
      ...
      if (node) ctx.startEdit({ type: 'rename', node, parentPath: parentPathOf(node.path), value: node.name });
    }
